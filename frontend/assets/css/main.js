...
function setLanguage(lang) {
  ...
  if (langToggle) {
    langToggle.textContent = translations[lang].lang_button;
  }

  const appLogin = document.getElementById('app-login');
  if (appLogin) {
    appLogin.innerHTML = `<i class="fas fa-user"></i> ${translations[lang].app_login}`;
  }
...
//       </div>
//       <div class="card-body">
//         <div class="role-small">${member[`role_${currentLang}`] || member.role_en}</div>
...
/* Slider controls for Team section */
let _teamCurrent = 0;
function initTeamSlider() {
  ...
  // build dots and attach click handlers; also attach click on card to focus it
  ...
  cards.forEach((card, i) => {
    ...
    btn.dataset.index = i;
    btn.addEventListener('click', (e) => { e.stopPropagation(); goToTeam(i, true); });
  ...
  window.goToTeam = function(i, focus=false) {
    ...
    if (card && track) {
      ...
      setActive(i);
      if (focus) {
        try { card.focus && card.focus(); } catch (e) {}
      }
    }
  };
  ...
  // update active dot when user scrolls (snap point reached)
  let scrollTimeout;
...
// Product Showcase Animation on Scroll
...
document.addEventListener('DOMContentLoaded', () => {
  setLanguage(currentLang);     // ← Keep your existing line
  renderTeam();                 // ← Keep your existing line
  initTeamSlider();             // ← Keep your existing line
